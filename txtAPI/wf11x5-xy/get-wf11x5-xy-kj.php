...
class vrsxffc
{
    ...
    public function __construct()
    {
        ...
        $this->api = 'https://luck.sdtiop.com/hall/pc/lottery/get-recent-close-expect.html?code=wf11x5';
        ...
        ////开奖时间00:05:00，每期5分钟  288期
        $this->nowTime=strtotime(date("Y-m-d H:i:s"));
        $this->kjTime=strtotime(date("Y-m-d")." 00:05:00");//	
        $this->jsTime=strtotime(date("Y-m-d")." 23:59:59");

        $isTEST=false;
  

        $this->cookieStr="SID=tEpYly7069pGCeU4cPuaUAgetv9Z+ChH4BjjWt+wiaYmQYk6Q0RNinJvjimcSzPRdLhM+YBXUcd0x7wNVsZDMAaXgON9vvwyR6JyCn17TR8=; route=76dc1d0dae34e97fe53cf8a76b2493be";
  ...
  //  $header = array("Connection: Keep-Alive","Accept: text/html, application/xhtml+xml, */*", "Pragma: no-cache", "Accept-Language: zh-Hans-CN,zh-Hans;q=0.8,en-US;q=0.5,en;q=0.3","User-Agent: Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; WOW64; Trident/6.0)",'CLIENT-IP:'.$ip,'X-FORWARDED-FOR:'.$ip);
    ...
    public function getSumSize($n)//总和值等于30为和 (不计算输赢)。
    {
        if ($n<30) {
            return "小";
        }elseif ($n==30) {
    ...
    public function getSumOddOrEven($n)
    {
        if ((abs($n)+2)%2==1) {
            return "单";
        }else {
    ...
    public function add($file_name, $str)
    {
        ...
        if ($this->nowTime < $this->kjTime) {
            ...
            echo  date('Y-m-d H:i:s', $this->nowTime).">".date('Y-m-d H:i:s',$this->kjTime)." AND do -1 day   <br> \n";
