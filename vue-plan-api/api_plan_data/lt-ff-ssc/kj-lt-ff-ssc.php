...
class vrsxffc
{
    ...
    public function __construct()
    {
        ...
        ////开奖时间，	00:01:00~~次日0:00:05    (实际在00:00:01左右抓到数据)
        ////计划于00:01:00进入访问，下方的开奖时间要比计划任务设定的早一点
        $this->nowTime=strtotime(date("Y-m-d H:i:s"));
        $this->kjTime=strtotime(date("Y-m-d")." 00:01:00");//	13:09:51
        $this->jsTime=strtotime(date("Y-m-d")." 00:00:59");

        $this->cookiePath=$dir."/cookie.txt";
        if ($isLuck) {
            $this->cookiePath=$dir."/cookieIsLuck.txt";
        }
        $this->api = 'https://www.ezun889.com/lottery/commonLottery/getRecent5Records.html?code=ffssc';
        if ($isLuck) {
            $this->api = 'https://luck.sdtiop.com/hall/pc/lottery/get-recent-close-expect.html?code=ffssc';
        }
