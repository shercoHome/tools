...
class vrsxffc
{
    ...
    public function __construct()
    {
        ...
        $this->kjInterval = 1*60;//每期1分钟  1440期
        $this->loopInterval = 2;
        $this->oneMID=5;//0-9  小于5的为小
        $this->sumMID=23;// 5位数，0~9，和值为0~45，小值为0-22   小于23的为小

        ////开奖时间，	09:02:00~~次日6:00:30    (实际在09:02:20左右抓到数据)
        ////计划于00:00:00进入访问，下方的开奖时间要比计划任务设定的早一点
        $this->nowTime=strtotime(date("Y-m-d H:i:s"));
        $this->kjTime=strtotime(date("Y-m-d")." 00:01:00");
        $this->jsTime=strtotime(date("Y-m-d")." 06:00:59");
        ...
        $this->api = 'http://evebs.vrbetapi.com/DrawHistory/GetRoadTrend';
        $lotteryGameID='45';//VR 腾讯分分彩
        $this->post_data=array(
            'lotteryGameID'=>$lotteryGameID,
            'quantity'=>'120',//几条数据
        );
        $this->referURL='http://evebs.vrbetapi.com/Bet/Index/'.$lotteryGameID;
        $this->origin='http://evebs.vrbetapi.com';


        $this->cookieStr="ASP.NET_SessionId=vwlheosxhzevycftcaef11ph; __RequestVerificationToken=LNjW3bNnT3LCadldDY4JUmHrHAcQJ4CQ14800RwA8idoeR506D6ChFclbtMtz_GMCB0P2ft6EsiRjBLN_ABcsnqzEPECap8vY5N9tVUVghs1; token=bd3569452ccfe91da6e2d2c4193f8ff4; random=9995; .AspNet.ApplicationCookie=2P0WEPr9Qii8t6gEDKfx0chzKsvHDm6Bs9vrRtfnoh9AHwNgV27ozUtChUOZmt_SCZtTch7OJ_rdEKNJlVisSoJtSWeMid-TVU2_ynJX6Vc3hPLpy_OcAL6xGGJmLXZzbWz30TF1mEa-8RzawaSCOgHDQwWmnaycJhHHzqmanWFdgRJbIb4gJsF4xTXKopnVtIKP4xdbXW6TsrYXRaDHF65mGZfwms42LC4yL6GPZY0QVpYI5b__OnRfTGqLsASs1F-Lm_ZwS0SurKOubbjVpfan8MyFyp5buXCxbNu6bv9fBOnL5Ed6Yhi1cuwWDvpG_c2v2cPnDe7FgQs90OGy_8NHTbFYVfluG5d2buLVhGgwpG15OnkEPzh3-Q1VQvJ5NtBk_Hf4kVGIZfnOwvNsa813esqZJNOnaPsUY_35snoJDNJjFyqveiHfBXys6Upy3HLBeVkyMRyFKqGCo8zeuKWjwuIBSkviq_h6Q9tkWCeZFzho0PvmcpYx2hxy3WFM9voLIWV7RzOEH_fMNk0YpeRe3RbZT6_BEeiCSWAaqYU";
    ...
    public function getApi()
    {
        ...
        if ($json!=null) {
            if (gettype($json)=="array") {
                ...
                $hzdx=$this->getSize($opencode_sum, $this->sumMID);//和值大小getSize  5位数，0~9，和值为0~45，小值为0-22
                ...
                $opencode = $str1."|||".$str2."|||".$str3."|||".$str4."|||".$str5."|||".$str6."|||".$str7."|||".$str8;
                ...
                if (property_exists($json, "data")) {
                    ...
                    //和值
                    $opencode_sum = array_sum($array_opencode);
                    $hzdx=$this->getSize($opencode_sum, $this->sumMID);//和值大小getSumSize  5位数，0~9，和值为0~45，小值为0-22
                    $hzds=$this->getOddOrEven($opencode_sum); //和值单双
                    $str4=$hzdx.",".$hzds.",".$hzdx.$hzds;
                    //龙虎和  以开奖结果的万位和个位作为基准，取万为龙，个为虎的数字进行大小比对的一种玩法
                    $str5=$this->getDragonOrTiger($array_opencode[0], $array_opencode[4]);
    ...
    public function postHtml()
    {
        ...
        $dir            = dirname(__FILE__);
        $cookie_file    = $dir . '/cookie_file.txt';
        $agent_array=[
            //PC�˵�UserAgent
