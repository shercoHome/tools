...
class vrsxffc
{
    ...
    public function __construct()
    {
        ...
        $this->referURL='http://www.cjzzc.com/wailian.html';
        $this->origin='http://www.cjzzc.com';
        $this->host='www.cjzzc.com';
        ...
        $resultStr="";
      // for($ii=201;$ii<=500;$ii++){
        $ii=281;
            $__pages=($ii-1)*10;
            $__pages.='';
            $this->api = 'http://www.cjzzc.com/wailian.html?page='.$ii;
            $lotteryGameID='myweb';
            $this->post_data=array(
                'url'=>$lotteryGameID,//VR水星分分彩
                'token_cjzzc'=>'6a9e4e6b5c7d946de7e038e83122a454',//几条数据
                'page'=>$__pages,//几条数据
            );
            $html=$this->getApi();
            ...
            if(count($matchest[0])>0){
                $pa='|<a .*</a>|isU';
                foreach ( $matchest[0] as $h3) {
                    preg_match_all($pa, $h3, $matchest_a);
                    if(count($matchest_a[0])>0){
                        $resultStr.="<br>".$matchest_a[0][0];
                        $count++;
                    }
                }
            }
