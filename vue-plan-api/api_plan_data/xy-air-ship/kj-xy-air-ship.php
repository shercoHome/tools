...
class vrsxffc
{
    ...
    public function __construct()
    {
        ...
        $this->kjInterval = 5*60;//每期1分钟  1440期
        $this->loopInterval = 2;
        $this->oneMID=6;//1-10  小于6的为小
        $this->sumMID=12;// 和值为3-19   小于12的为小

        ////开奖时间，13:09:50~~次日4:04:50    (实际在13:09:51左右抓到数据)
        ////计划于13:09:45进入访问，下方的开奖时间要比计划任务设定的早一点
        $this->nowTime=strtotime(date("Y-m-d H:i:s"));
        $this->kjTime=strtotime(date("Y-m-d")." 13:09:40");//	13:09:51  
        $this->jsTime=strtotime(date("Y-m-d")." 04:05:00");

        $this->cookiePath=$dir."/cookie.txt";
        if ($isLuck) {
            $this->cookiePath=$dir."/cookieIsLuck.txt";
        }
        $this->api = 'https://www.ezun889.com/lottery/commonLottery/getRecent5Records.html?code=xyft';
        if ($isLuck) {
            $this->api = 'https://luck.sdtiop.com/hall/pc/lottery/get-recent-close-expect.html?code=xyft';
        }
